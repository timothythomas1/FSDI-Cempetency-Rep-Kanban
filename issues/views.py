...
class IssueCreateViewCustomer(LoginRequiredMixin, CreateView, CustomUser):
    template_name = "issues/new_customer.html" 
    model = Issue
    success_url = reverse_lazy("issue_list")
    fields = ["title", "summary", "description", "impact", "urgency"]
...
class IssueCreateView(LoginRequiredMixin, CreateView, CustomUser):
...
class IssueWorkView(LoginRequiredMixin, UpdateView):  # REMEMBER, Mixin order matters for validations.
    template_name = "issues/issue_work.html"
    model = Issue
    success_url = reverse_lazy("issue_list") 
    fields = ["assignee", "status"] # Fields will be showed on the page
    # Test for UserPassesTestMixin that the user needs to pass
    # def test_func(self):
    #     issue_obj = self.get_object()
    #     return issue_obj.requester == self.request.user